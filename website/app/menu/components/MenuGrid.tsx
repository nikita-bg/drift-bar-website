...
import { getPlaceholderTypeForCategory } from '@/lib/generate-placeholder'
...
export default function MenuGrid({ items, categoryType, categoryKey }: MenuGridProps) {
    if (items.length === 0) {
        return (
    ...
    const placeholderType = getPlaceholderTypeForCategory(categoryKey)
        ...
        <div className={categoryType === 'featured' ? styles.gridFeatured : styles.gridSimple}>
            {items.map((item) => (
                <MenuItem
                    key={item.id}
                    item={item}
                    featured={categoryType === 'featured'}
